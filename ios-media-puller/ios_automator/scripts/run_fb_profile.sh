#!/usr/bin/env bash
# Full pipeline: preflight → stack → Facebook Home → Profile (name/friends/posts/followers).
set -euo pipefail
...
export ROOT
export RUN_LABEL="run_fb_profile"
...
python ios_automator/automator.py --skip-wda-install fb-profile --http "http://127.0.0.1:${WDA_PORT:-8100}"
exit $?
