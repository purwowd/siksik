...
const RECOVERY_LABELS: Record<GalleryItem["recovery_state"], string> = {
  normal: "Data normal",
...
function sourceLabel(item: GalleryItem): string {
  if (item.recovery_state === "trash") return "Trash perangkat";
  if (item.recovery_state === "recovered_deleted") return "Recovery cache/thumbnail";
  return (
    (item.source_app && SOCIAL_LABELS[item.source_app]) || humanLabel("source", item.source)
  );
}
...
function ItemMeta({ item }: { item: GalleryItem }) {
  return (
    <>
      <div className={`finding-meta gallery-state gallery-state-${item.recovery_state}`}>
        {RECOVERY_LABELS[item.recovery_state]}
      </div>
      {item.artifact_role ? (
