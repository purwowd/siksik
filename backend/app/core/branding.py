...
# Canonical crawl-record MIME (writers prefer this).
CANONICAL_CRAWL_RECORD_MIME = "application/vnd.satria.crawl-record+json"
# Legacy MIME still accepted on read (Android agent / older staging).
...
def session_id_field(**kwargs: Any) -> Any:
    """Wire field: accept satria_session_id or siksik_session_id; serialize as siksik_session_id."""
    return Field(
...
def promote_satria_env(*, root: Path | None = None) -> None:
    """Load SATRIA_* from .env into os.environ, then copy SATRIA_* → SADT_* (SATRIA wins).

    Existing ``SADT_*``-only deployments keep working. When both prefixes are set for
    the same key, ``SATRIA_*`` takes priority.
    """
