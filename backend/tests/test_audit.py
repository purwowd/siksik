...
async def test_session_audit_and_authorize_hash(client: AsyncClient, anon_client: AsyncClient):
    ...
    report = await client.get(f"/api/v1/sessions/{sid}/report?format=json")
    assert report.status_code == 200
    trail_after = (await client.get(f"/api/v1/sessions/{sid}/audit")).json()
    assert any(row["action"] == "report_downloaded" for row in trail_after)
